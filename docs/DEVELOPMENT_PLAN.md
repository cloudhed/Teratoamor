...
- [x] Add Hold, Decay, and Sustain as a provisional envelope (`el<N>_hold`, `el<N>_decay`, `el<N>_sustain`, version hint 4; Attack -> Hold -> Decay -> Sustain -> Release in `ElementEnvelope.h`). No reference recordings exist for them: the 10-units-per-second scale, linear decay to the sustain level, and defaults (Hold 0, Decay 5, Sustain 100) are assumptions. Values are read at note-on.
- [x] Add Time as a provisional Decay and Release length control (`el<N>_time`, version hint 4, default 50): both lengths are multiplied by Time/50, with a floor of 0.1 at Time 0. Fitted to the `07_time_*` recordings: the default Release tail (about 0.05 s, 0.45 s, 0.8 s at Time 0, 50, 100), Release 25 (about 0, 2.3 s, 4.7 s), and Decay 50 with Sustain 0 (fades in about 0.5 s, 3.5 s, 6.5 s). Onset was assumed unaffected (the slower onset in the Time 100 file is probably MIDI keypress timing). Decay was re-scaled to about 0.066 s per unit (Decay 50 is about 3.3 s at Time 50) because the recording faded faster than 5 s. Hold is not scaled by Time (unmeasured).
- [ ] Record reference batches for Hold, Decay, and Sustain on their own (see `CHIMERA_REFERENCE.md`), with the note held long enough to see the whole shape, then refine the provisional stages (decay curve shape, Hold and Sustain behaviour, whether Time affects Hold).
